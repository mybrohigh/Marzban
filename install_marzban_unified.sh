...
# Unified Marzban Installation Script with Advanced Limits System
# Single command for both Docker and Native installation
# Solves all argument length and curl issues
...
detect_install_mode() {
    if command -v docker >/dev/null 2>&1; then
        if [[ -f "/var/lib/marzban/app.db" ]] || [[ -d "/opt/marzban/app/db" ]]; then
            INSTALL_MODE="docker"
            print_color "$YELLOW" "Auto-detected Docker installation mode"
        else
            INSTALL_MODE="native"
            print_color "$BLUE" "Auto-detected Native installation mode (with limits system)"
        fi
    else
...
# Docker installation
install_docker() {
    print_color "$GREEN" "Installing Marzban (Docker mode)..."
...
# Native installation
install_native() {
    print_color "$GREEN" "Installing Marzban with Advanced Limits System (Native mode)..."
...
# Generated by unified installer on $(date)
...
EOF
    ...
    print_color "$GREEN" "✓ Marzban with Advanced Limits installed successfully!"
    echo ""
    ...
    echo ""
    print_color "$GREEN" "🚀 Marzban with Advanced Limits is ready to use!"
}

# Help function
show_help() {
    echo "Marzban Unified Installation Script with Advanced Limits System"
    echo ""
    echo "This script automatically detects the best installation method:"
    echo "- Docker mode if Docker is available and database doesn't exist"
    echo "- Native mode with limits system otherwise"
    echo ""
    echo "Usage: $0 [COMMAND]"
    echo ""
    echo "Commands:"
    echo "  install    - Install Marzban with Advanced Limits System"
    ...
    echo "  - Solves curl argument length issues"
    echo "  - Creates systemd service and configuration"
    echo "  - Database initialization"
    echo "  - Advanced limits system integration"
}
    ...
    case "${1:-install}" in
        install)
            ...
            if [[ "$INSTALL_MODE" == "docker" ]]; then
                ...
                print_color "$RED" "Error: Unknown installation mode: $INSTALL_MODE"
                exit 1
            fi
            ;;
        help|--help|-h)
