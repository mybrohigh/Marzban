# 🚀 Установка Marzban с системой лимитов (без Docker)

## 📋 Что это дает:

### ✅ **Полная система лимитов:**
- 📊 **База данных** - SQLite с миграциями
- 🎛️ **Управление** - CRUD API для лимитов
- 📋 **Шаблоны** - Basic/Premium/Enterprise тарифы
- ⚡ **Мониторинг** - автоматическая проверка каждые 5 минут
- 🔔 **Уведомления** - email/telegram/webhook
- 📈 **Статистика** - детальная аналитика

### 🔄 **Два режима установки:**
1. **Docker режим** - оригинальный образ `gozargah/marzban`
2. **Native режим** - установка с системой лимитов

## 🛠️ Способы установки:

### **Способ 1: Онлайн установка (рекомендуется)**
```bash
# Установить последнюю версию с системой лимитов
sudo bash -c "$(curl -sL https://github.com/mybrohigh/Marzban/raw/master/install_marzban_with_limits.sh)" @ install --native

# Или использовать оригинальный образ (без лимитов)
sudo bash -c "$(curl -sL https://github.com/mybrohigh/Marzban/raw/master/marzban.sh)" @ install
```

### **Способ 2: Локальная установка**
```bash
# 1. Скачать установочный скрипт
curl -O install_marzban_with_limits.sh \
  https://github.com/mybrohigh/Marzban/raw/master/install_marzban_with_limits.sh

# 2. Сделать исполняемым
chmod +x install_marzban_with_limits.sh

# 3. Запустить установку
sudo ./install_marzban_with_limits.sh install --native
```

### **Способ 3: Из исходного кода**
```bash
# 1. Клонировать репозиторий
git clone https://github.com/mybrohigh/Marzban.git
cd Marzban

# 2. Запустить native установку
sudo bash marzban.sh install --native
```

## 🎯 Что установится:

### **Системные компоненты:**
- ✅ **Python 3.12** с виртуальным окружением
- ✅ **SQLite база данных** с автоматическими миграциями
- ✅ **Systemd сервис** для автозапуска
- ✅ **Nginx** (опционально) для reverse proxy
- ✅ **Log rotation** для управления логами

### **Файлы и директории:**
```
/opt/marzban/                    # основная директория
├── venv/                         # Python окружение
├── app.db                         # База данных SQLite
├── .env                           # Конфигурация
├── marzban.sh                     # Скрипт управления
└── requirements.txt                # Зависимости Python

/var/lib/marzban/                  # Данные приложения
├── app.db                         # База данных
├── xray_config.json               # Конфиг Xray
└── logs/                          # Логи приложения

/etc/systemd/system/marzban.service  # Systemd сервис
/etc/logrotate.d/marzban              # Настройка ротации логов
```

### **Конфигурация (.env):**
```bash
# База данных
SQLALCHEMY_DATABASE_URL=sqlite:///var/lib/marzban/app.db

# Приложение
DEBUG=false
UVICORN_HOST=0.0.0.0
UVICORN_PORT=8000

# Система лимитов
LIMITS_MONITOR_ENABLED=true
LIMITS_CHECK_INTERVAL=300
LIMITS_NOTIFICATION_THRESHOLD=0.8

# Безопасность
JWT_SECRET_KEY=ваш_секретный_ключ
ADMIN_USERNAME=admin
ADMIN_PASSWORD=сгенерированный_пароль
```

## 🚀 Управление после установки:

### **Основные команды:**
```bash
# Статус сервиса
sudo systemctl status marzban

# Просмотр логов
sudo journalctl -u marzban -f

# Перезапуск
sudo systemctl restart marzban

# Остановка
sudo systemctl stop marzban

# Обновление
sudo bash /opt/marzban/marzban.sh update

# Удаление
sudo bash /opt/marzban/marzban.sh uninstall
```

### **Управление лимитами:**
```bash
# Доступ к CLI
sudo /opt/marzban/venv/bin/python /opt/marzban/main.py cli

# Или через веб-интерфейс
http://your-server:8000
```

## 📊 Сравнение режимов:

| Характеристика | Docker режим | Native режим |
|---------------|-------------|-------------|
| Установка | 1 команда | 1 команда |
| Ресурсы | Контейнер | Система |
| Портативность | Высокая | Максимальная |
| База данных | Внутри | Внешняя |
| Конфигурация | .env | .env |
| Лимиты | Базовые | Расширенные |

## 🎯 Преимущества Native режима:

### **Для бизнеса:**
- 💰 **Полный контроль** - детальная настройка лимитов
- 📈 **Аналитика** - статистика использования ресурсов
- ⚡ **Производительность** - нативная скорость работы
- 🔧 **Гибкость** - индивидуальные настройки
- 💾 **Бэкапы** - автоматические бэкапы БД

### **Для администрирования:**
- 🎛️ **Мониторинг** - real-time отслеживание
- 📋 **Шаблоны** - быстрая конфигурация
- 🔍 **Диагностика** - детальные логи
- ⚙️ **Интеграции** - webhook и уведомления

## ⚠️ Требования:

### **Системные:**
- Ubuntu 18.04+ / Debian 10+ / CentOS 8+ / Fedora 35+
- Минимум 2GB RAM
- Минимум 10GB дискового пространства

### **Сетевые:**
- Открытый порт 8000
- Доступ в интернет для загрузки зависимостей

## 🎉 Результат:

**У вас есть выбор:**
1. **Docker режим** - просто и надежно
2. **Native режим** - полнофункционально

**Оба варианта полностью рабочие!** Выбирайте под ваши задачи.
