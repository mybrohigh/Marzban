...
install_marzban_script() {
    FETCH_REPO="mybrohigh/Marzban"
    SCRIPT_URL="https://github.com/$FETCH_REPO/raw/master/marzban.sh"
    colorized_echo blue "Installing marzban script"
    curl -sSL $SCRIPT_URL | install -m 755 /usr/local/bin/marzban
    colorized_echo green "marzban script installed successfully"
}

install_marzban_native() {
    colorized_echo blue "Installing Marzban with Advanced Limits System (Native Mode)..."
    
    # Create installation directory
    if [[ -d $APP_DIR ]]; then
        colorized_echo yellow "Directory $APP_DIR already exists. Backing up..."
...
Environment=SQLALCHEMY_DATABASE_URL=sqlite:///$DATA_DIR/app.db
...
EOF
    ...
    if [[ ! -f $ENV_FILE ]]; then
        colorized_echo blue "Creating configuration file..."
        cat > $ENV_FILE << EOF
# Marzban Configuration with Advanced Limits System
# Generated by install script on $(date)

# Database Configuration
SQLALCHEMY_DATABASE_URL=sqlite:///$DATA_DIR/app.db
...
EOF
        colorized_echo green "✓ Configuration file created"
        colorized_echo yellow "⚠ Please edit $ENV_FILE with your settings"
    fi
    
    # Start service
    colorized_echo blue "Starting Marzban service..."
    systemctl start ${SERVICE_NAME}
    systemctl status ${SERVICE_NAME}
    
    colorized_echo green "✓ Marzban with Advanced Limits installed successfully!"
    echo ""
    colorized_echo cyan "Web Interface: http://$(hostname -I | awk '{print $1}'):8000"
    colorized_echo cyan "Installation Directory: $APP_DIR"
    colorized_echo cyan "Configuration File: $ENV_FILE"
    echo ""
    colorized_echo yellow "Next Steps:"
    colorized_echo yellow "1. Edit $ENV_FILE with your settings"
    colorized_echo yellow "2. Access web interface to configure limits"
    colorized_echo yellow "3. Create users and apply templates"
    echo ""
    colorized_echo green "🚀 Marzban with Advanced Limits is ready to use!"
}
...
install_marzban() {
    ...
    # Fetch releases
    FILES_URL_PREFIX="https://raw.githubusercontent.com/mybrohigh/Marzban/master"
...
services:
  marzban:
    image: ${MARZBAN_IMAGE}:${marzban_version}
    restart: always
    env_file: .env
    network_mode: host
    volumes:
...
EOF
        ...
        if [ "$marzban_version" == "latest" ]; then
            yq -i ".services.marzban.image = \"${MARZBAN_IMAGE}:latest\"" "$docker_file_path"
        else
            yq -i ".services.marzban.image = \"${MARZBAN_IMAGE}:${marzban_version}\"" "$docker_file_path"
        fi
...
install_command() {
    ...
    check_version_exists() {
        local version=$1
        repo_url="https://api.github.com/repos/mybrohigh/Marzban/releases"
...
usage() {
    ...
    colorized_echo yellow "  install         $(tput sgr0)– Install Marzban (Docker mode)"
    colorized_echo yellow "  install --native $(tput sgr0)– Install Marzban with limits (Native mode)"
